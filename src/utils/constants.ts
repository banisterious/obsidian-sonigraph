...
export interface SonigraphSettings {
	...
	exportSettings?: ExportSettings; // Export feature settings
	effects?: {
		[key: string]: {
			enabled: boolean;
			[param: string]: any;
	...
	// Phase 7.2: User's Freesound sample library (flat array, no genre organization)
	freesoundSamples?: Array<{
		id: number;
		title: string;
		previewUrl: string;
		duration: number;
	...
	localSoundscape?: {
		...
		// Note-centric musicality settings
		noteCentricMusicality?: {
			preset?: 'conservative' | 'balanced' | 'adventurous' | 'custom'; // Preset selection (default: 'balanced')
			// Individual controls (active when preset is 'custom')
			timingHumanization?: number; // Micro-timing variation in ms (50-250, default: 125)
			harmonicAdventurousness?: number; // Harmonic color frequency (0-100, default: 75)
			dynamicRange?: 'subtle' | 'moderate' | 'extreme'; // Velocity variation (default: 'extreme')
			polyphonicDensity?: 'sparse' | 'moderate' | 'maximum'; // Embellishment overlap (default: 'maximum')
			melodicIndependence?: number; // Embellishment freedom from center (0-100, default: 80)
			voiceLeadingStyle?: 'smooth' | 'balanced' | 'chromatic'; // Voice leading approach (default: 'chromatic')
		};
	};
}
...
export const DEFAULT_SETTINGS: SonigraphSettings = {
	...
	instruments: {
		...
		whaleBlue: {
			...
			volume: 0.8,
			maxVoices: 1,
			useHighQuality: true, // Recordings-only instrument (no synthesis available)
			effects: {
				reverb: {
					enabled: true,
					params: {
	...
	audioEnhancement: {
		...
		noteCentricMusicality: {
			preset: 'balanced', // Balanced preset matches current hardcoded implementation
			// Individual control defaults (used when preset is 'custom')
			timingHumanization: 125, // ±125ms matches current ±100-150ms average
			harmonicAdventurousness: 75, // Moderate harmonic color frequency
			dynamicRange: 'extreme', // Full 0.08-0.99 velocity range
			polyphonicDensity: 'maximum', // Dense embellishment overlap (0-2s delays)
			melodicIndependence: 80, // High independence with 20% center influence
			voiceLeadingStyle: 'chromatic' // Chromatic voice leading by default
		}
	},
