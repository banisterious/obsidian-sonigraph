...
export class FreesoundSampleLoader {
  ...
  private initializeSampleLibrary(): void {
    // Initialize with curated samples from the Freesound Audio Library document
    // This would be populated from the actual library data
    ...
    // Initialize other genres with placeholder data
    const remainingGenres: MusicalGenre[] = [
      'orchestral', 'minimal', 'oceanic', 'sci-fi', 'experimental',
      'urban', 'nature', 'mechanical', 'organic'
    ];
    
    remainingGenres.forEach(genre => {
      this.sampleLibrary.set(genre, []); // Empty for now - would be populated from actual library
    });
